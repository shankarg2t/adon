...
if(!defined('MagicMagnifyPlusModuleCoreClassLoaded')) {

    define('MagicMagnifyPlusModuleCoreClassLoaded', true);

    require_once(dirname(__FILE__) . '/magictoolbox.params.class.php');

    /**
     * MagicMagnifyPlusModuleCoreClass
     *
    ...
    class MagicMagnifyPlusModuleCoreClass {
        ...
        function MagicMagnifyPlusModuleCoreClass() {
            ...
            $this->params->setMapping(array(
                ...
                'lense-url' => array('' => null),
                'lense-offset-x' => create_function('&$params', 'return $params->checkValue("lense-url","")?null:$params->getValue("lense-offset-x");'),
                'lense-offset-y' => create_function('&$params', 'return $params->checkValue("lense-url","")?null:$params->getValue("lense-offset-y");'),
                'lense-position' => create_function('&$params', 'return $params->checkValue("lense-url","")?null:$params->getValue("lense-position");'),
                'disable-auto-start' => array('Yes' => 'true', 'No' => 'false'),
                'hide-cursor' => array('Yes' => 'true', 'No' => 'false'),
        ...
        function getHeadersTemplate($jsPath = '', $cssPath = null) {
            ...
            if(!defined('MagicMagnifyPlusModuleHeaders')) {
                define('MagicMagnifyPlusModuleHeaders', true);
            } else {
                return '';
            }
    ...
    function MagicThumb_selectorClick(el) {
        ...
        function getOptionsTemplate() {
            ...
            return "<script type=\"text/javascript\">\n\tMagicMagnifyPlus.options = {{$addition}\n\t\t".$this->params->serialize(true, ",\n\t\t")."\n\t}\n</script>";
        }
        ...
        function getMainTemplate($params) {
            ...
            if(!empty($link)) {
                if($this->params->checkValue('disable-expand', 'Yes')) {
                    //if MagicThumb disabled
                    $rel .= 'link-url:'.($link).';hide-cursor:false;';
                } else {
                    $rel .= 'link:'.($link).';';
                }
            }
            ...
            return "<a class=\"MagicMagnifyPlus\"{$title} id=\"MagicMagnifyPlusImage{$id}\" href=\"{$img}\" {$rel}><img{$width}{$height} src=\"{$thumb}\" alt=\"{$alt}\" />{$description}</a>{$message}";
        }
        ...
        function getSelectorTemplate($params) {
            ...
            if(empty($title) || $this->params->checkValue('show-caption', 'No')) {
                $title = '';
            } else {
                $title = htmlspecialchars(htmlspecialchars_decode($title, ENT_QUOTES));
            ...
            return "<a {$title} rel=\"magnifier-id:MagicMagnifyPlusImage{$id};caption-source:a:title;\" href=\"{$img}\" rev=\"{$medium}\"><img{$width}{$height} src=\"{$thumb}\" alt=\"{$alt}\" /></a>";
        }
